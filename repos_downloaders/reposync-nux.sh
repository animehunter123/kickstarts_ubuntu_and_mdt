...
#0. Make a LM-NAS folder: mkdir /mnt/INSERT_YOUR_NFSSHARE_HERE/repos/nux/
...
nohup_log="/mnt/INSERT_YOUR_NFSSHARE_HERE/repos/logs/$(basename -- "$0").$(date +'%Y%m%d-%H%M').log"
echo "Starting \`reposync\` of nux-misc... (DO NOT PRESS CTRL C)."
echo "Please tail the log file via:        tail -f $nohup_log"
cd /mnt/INSERT_YOUR_NFSSHARE_HERE/repos/nux/
nohup reposync -r nux-misc -p /mnt/INSERT_YOUR_NFSSHARE_HERE/repos/nux/      2>&1 1>$nohup_log 
echo "Finished nux-misc."
echo "Starting reposync of nux-dextop..."
# WAIT FOR THIS TO FINISH before running the next reposync!!!!!!!!!! !! !!
nohup reposync -r nux-dextop -p /mnt/INSERT_YOUR_NFSSHARE_HERE/repos/nux/   2>&1 1>>$nohup_log  &    
echo "Finished nux-misc."
