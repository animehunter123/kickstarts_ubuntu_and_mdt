...
#0. Make a INSERT_YOUR_PASS_HERE folder: mkdir /mnt/MYSITENAS-Data/repos/nux/
...
nohup_log="/mnt/MYSITENAS-Data/repos/logs/$(basename -- "$0").$(date +'%Y%m%d-%H%M').log"
echo "Starting \`reposync\` of nux-misc... (DO NOT PRESS CTRL C)."
echo "Please tail the log file via:        tail -f $nohup_log"
cd /mnt/MYSITENAS-Data/repos/nux/
nohup reposync -r nux-misc -p /mnt/MYSITENAS-Data/repos/nux/      2>&1 1>$nohup_log 
echo "Finished nux-misc."
echo "Starting reposync of nux-dextop..."
# WAIT FOR THIS TO FINISH before running the next reposync!!!!!!!!!! !! !!
nohup reposync -r nux-dextop -p /mnt/MYSITENAS-Data/repos/nux/   2>&1 1>>$nohup_log  &    
echo "Finished nux-misc."
