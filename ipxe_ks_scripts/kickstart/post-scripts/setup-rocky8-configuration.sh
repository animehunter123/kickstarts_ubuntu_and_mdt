...
echo "@@ Setting Rocky 8 Configuration..."
set +e # disable exit on error for this bash script

echo "@@ Setting Rocky 8 History size to 800000..."
echo 'export HISTSIZE=800000' >> /etc/bashrc

echo "@@ Setting Rocky 8 Repos..."
pushd .
cd /etc/yum.repos.d/
...
curl http://INSERT_YOUR_WEBSERVER_HERE.YOURDOMAIN.COM/http-git/kickstart/pre-scripts/rocky8-lmnas-all-yum.repo -o rocky8-lmnas-all-yum.repo
yum clean all
yum repolist
popd 

echo "@@ Setting Rocky 8 Resolv.conf..."
pushd .
cd /etc
cat <<EOF > resolv.conf
...
# Rocky8 has firewalld by default, so disable it.
